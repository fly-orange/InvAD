...
class Pyramid(nn.Module):
    ...
    def get_scores(self, x):
        ...
        # scores = torch.sigmoid(self.scorenet(out)).squeeze(dim=2) # (B, L1+L2,..., Ls)
               
        # ret['output'] = out
        # ret['scores'] = [scores[:, sum(self.effe_size[:i]):sum(self.effe_size[:i+1])] for i in range(self.depth)]  # [(B, L1), (B, L2), ..., (B, Ls)]
    ...
    #     h = self.fc(out).squeeze(dim=2) # (B, T)
    ...
    #         actmap = actmap / actmax.unsqueeze(dim=1)   # (B, T)
    #         # Sequential labels
