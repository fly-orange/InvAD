...
class NTXentLoss_poly(torch.nn.Module):
    ...
    def forward(self, zis, zjs):
        ...
        # print(similarity_matrix.shape, l_pos.shape)
        ...
        negatives = similarity_matrix[mask_samples_from_same_repr].view(2 * batch_size, -1) # (2N, 2N)

        logits = torch.cat((positives, negatives), dim=1)
        logits /= self.temperature

        """Criterion has an internal one-hot function. Here, make all positives as 1 while all negatives as 0. """
        labels = torch.zeros(2 * batch_size).to(self.device).long() # (2N)
