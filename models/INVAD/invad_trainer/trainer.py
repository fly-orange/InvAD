...
def Trainer(args, model, optimizer, train_loader, valid_loader, test_loader, device, logger, configs):
    ...
    for epoch in range(configs.n_epochs):
        ...
        speed = (end - start)/len(train_loader)
        print(f'Train per iter {speed}s')
        # Evaluate the model using the validation set
        valid_result = model_evaluate(model, valid_loader, device, configs, train_loader) # to stably obtain the global_threshold
        ...
        test_result = model_evaluate(model, test_loader, device, configs, None)
        ...
        print('\tTest (DENSE) AUC : {:.3f}, AUPRC : {:.3f}, F1 : {:.3f}, Precision : {:.3f}, Recall : {:.3f}, IoU : {:.3f}'.format(
            test_result['dauc'], test_result['dauprc'], test_result['dbf1'], test_result['dbprecision'], test_result['dbrecall'], test_result['dbIoU'])) # dependent on fixed threshold
    ...
    valid_result = model_evaluate(model, valid_loader, device, configs, None)
    model.global_threshold = valid_result['global_threshold']
    test_result = model_evaluate(model, test_loader, device, configs, None)
    ...
    return test_result['wauc'], test_result['dauc']

def model_evaluate(model, data_loader, device, configs, train_loader=None):
    model.eval()
    total, total_loss, total_bce_loss, total_inn_loss = 0, 0, 0, 0
    wscores_cls, wscores_rec, wlabels = [], [], []
    dscores_cls, dscores_rec, dlabels = [], [], []
    ...
    for itr, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            dpred = model.get_dpred(out[3], out[2]) # dense决策依赖于weak决策
            dresult += compute_dacc(dpred, dlabel)
            dscores_cls.append(out[4])
            dscores_rec.append(rec_score)
    ...
    wscores_cls, wscores_rec, wlabels = torch.cat(wscores_cls, dim=0), torch.cat(wscores_rec, dim=0), torch.cat(wlabels, dim=0)
    dscores_cls, dscores_rec, dlabels = torch.cat(dscores_cls, dim=0), torch.cat(dscores_rec, dim=0), torch.cat(dlabels, dim=0)
    wscores_rec = (wscores_rec - wscores_rec.min())/(wscores_rec.max() - wscores_rec.min())
    dscores_rec = (dscores_rec - dscores_rec.min())/(dscores_rec.max() - dscores_rec.min())
    wscores = (1-configs.recoef)*wscores_cls + configs.recoef*wscores_rec
    dscores = (1-configs.recoef)*dscores_rec + configs.recoef*dscores_rec
...
def train(out, wlabel, aug, rec_score, configs, device):
    ...
    loss = loss_semi +  configs.lambda1 * loss_inn + configs.lambda2 * loss_self
